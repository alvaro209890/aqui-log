...
| Dashboard React | KPIs, entregas (+ categoria/tamanho/peso/cliente), mapa, motoboys, usuários, auditoria, configurações e relatórios | QA browser e identidade laranja pendentes |
...
`B2C-01B` está `IN_PROGRESS` (filtros de código ok). Falta **QA browser**
**ou** executar `BASE-04` (ainda `READY`).
